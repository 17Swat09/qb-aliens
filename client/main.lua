...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function(xPlayer)
	...
	players = {}
		...
		RegisterNetEvent("qb-aliens:playerupdate")
		AddEventHandler("qb-aliens:playerupdate", function(mPlayers)
			players = mPlayers
		end)
	...
	entitys = {}
		...
		TriggerServerEvent("qb-aliens:newplayer", PlayerId())
		...
		AddEventHandler("ZombieSync", function()
			...
			SetRelationshipBetweenGroups(2, GetHashKey("PLAYER"), GetHashKey("zombie"))
				while true do
					...
					if #entitys < Config.SpawnZombie then
						...
						-- Give weapon randomly
						if (math.random(1, 2) == 1 and isMartian) then
							GiveWeaponToPed(entity, GetHashKey("WEAPON_RAYPISTOL"), 5, false, true)
						end
				...
				while true do
					...
					for i, entity in pairs(entitys) do
						for j, player in pairs(players) do
							...
							if distance <= 50.0 then --------------------------------change distance
								...
								if GetPedSourceOfDeath(entity) == PlayerPedId() then
									...
									if not IsPedInAnyVehicle(PlayerPedId(), false) then
										if(Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 4.0) then
											QBCore.Functions.DrawText3D(pedX, pedY, pedZ + 0.2, 'Examinar - [~g~E~w~]')
			...
			RegisterNetEvent('qb-aliens:clear')
			AddEventHandler('qb-aliens:clear', function()
				for i, entity in pairs(entitys) do
					local model = GetEntityModel(entity)
					SetEntityAsNoLongerNeeded(entity)
					SetModelAsNoLongerNeeded(model)
